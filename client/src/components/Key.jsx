...
export default function Keys() {
  ...
  return (
    <div className="min-h-screen bg-black text-white  px-6 py-12 flex flex-col items-center">
      <h1 className="text-3xl  mb-6">Wallet Keys</h1>
      ...
      {!wallet ? (
        ...
        <div className="w-full max-w-3xl bg-white/5 p-6 rounded-lg shadow-lg space-y-6">
          <div>
            ...
            <p className="break-all  bg-white/10 p-2 rounded">{wallet.publicKey}</p>
          </div>
          <div>
            <p className="text-gray-400 text-sm mb-1">Private Key:</p>
            <p className="break-all text-red-300 bg-white/10 p-2 rounded">{wallet.privateKey}</p>
          </div>

          <button className="mt-4 bg-yellow-500 hover:bg-yellow-600 px-6 py-2 rounded-lg  text-black">
            Add Wallet
          </button>
        </div>
      )}
